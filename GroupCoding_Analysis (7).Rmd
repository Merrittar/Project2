...
author: "Kira Ioffe, Mia Corcino, Ayden Passaretti, Lea Merritt"
...
NFL <- 
  read_csv("https://aokine.github.io/Data/NFLplayers2016.csv")
...
```{r BMIcalculation}
NFL <- NFL %>%
  mutate(kg = Weight*0.453592, m = Height*0.0254, BMI = kg/(m^2))
```
...
```{r factorconvert}
NFL$Playtime <- factor(NFL$Playtime, ordered = TRUE, levels = c("Rookie", "1-4 Yrs", "5-8 Yrs", "9-12 Yrs", "13+ Yrs"), labels = c("Rookie", "1-4 Years", "5-8 Years", "9-12 Years", "13+ Years"))
NFL %>% count(Playtime)
...
```{r heighthistogram}
# add scale to the x and y axis
ggplot(NFL, aes(Height))+
  geom_histogram(binwidth=1, color = "black", fill = "olivedrab3") + 
  annotate("rect", xmin=64.0, xmax=73.5, ymin=0, ymax=400, alpha=.2, fill = "steelblue1" ) + 
  annotate("rect", xmin=67.0, xmax=71.0, ymin=0, ymax=400, alpha=.3, fill = "steelblue") + 
  geom_vline(xintercept =  69.2, linetype = "dashed", color = "black") + 
  labs(title="Height Distribution of NFL Players ", x="Height of NFL PLayers(in inches)", y="Count of NFL Players") + 
  theme_minimal()
...
**ANSWER:** 
The median height of all US men in 2016 is denoted by a black dashed line, and compared to the distribution of NFL players, the average man falls on the lower end of the distribution. The median height of all US men is 69.50, whereas the median height of an NFL player is 74, and the mode height is 75, so there is around a 5-6 inch difference. The middle 90% of men fall in the bottom 45% of NFL players, meaning that over 50% of NFL players are taller than the average man. To say an overall comparison, it is clear that the height of NFL players is higher than the average man. I think that it would have been useful to have another histogram of all us men's heights, and compare the histograms side to side. That could give us an easier comparison. 
...
```{r BMIdensity}
ggplot(NFL, aes(BMI)) +
geom_density(color = "darkblue", fill = "lightblue", alpha = .7) +
geom_vline(xintercept = c(18.5, 25, 30, 35, 40), color = "grey", linetype = 4) +
annotate("text", x = 22, y = .000, label = "Healthy", alpha = .8, size = 3.5) +
annotate("text", x = 27.5, y = .000, label = "Overweight", alpha = .8, size = 3.5) +
annotate("text", x = 32.5, y = .000, label = "Obese", alpha = .8, size = 3.5) +
annotate("text", x = 37.5, y = .000, label = "Obese(Class I)", alpha = .8, size = 3.5) +
annotate("text", x = 43, y = .000, label = "Obese(Class II)", alpha = .8, size = 3.5) +
labs(title = "NFL BMI Rates", x = "BMI", y = "Density")+
theme_minimal()
...
**ANSWER:** The plots show that BMI is indeed a poor measure for athletes. Most information suggests that most athletes are overweight and nearing obesity. The graph spikes at both overweight and again at Class I obesity. Unfortunately, BMI does not include the measurement of muscle mass, instead considering body fat and muscle mass to be the same. Due to this, BMI is often an inaccurate measurement for athletes.      
...
ggplot(NFL, aes(Playtime)) +
  geom_bar(fill = "blue", color = "black") + 
  scale_y_continuous(breaks = seq(0, max(table(NFL$Playtime)) + 50, by = 100)) +
  labs(
    title = "Distribution of Play time",
    x = "Play time in Yrs",
    y = "# of Players",
...
```{r ageviolins}
ggplot(NFL, aes(x = Age, y = Side)) + geom_violin() + geom_boxplot(width = 2, fill = "blue", outlier.size = 6) + 
         labs(title= "Age and Side Groups",
              x = "Yards",
              y = "Position")
...
```{r yearsbeeswarm}
library(ggbeeswarm)
ggplot(NFL, aes(x = Years, y = Side)) +
  geom_beeswarm() +
  labs(title = "Career Time",
       x = "Years",
       y = "Position") +
  theme_minimal()
...
**ANSWER:**  Based on the graphs, it looks like most NFL players only last 1-4 years. Very few NFL players make it past 13+ years.  The three plots highlight the relationship between players position and the amount of time they spend in the NFL/ their performance while doing so.
